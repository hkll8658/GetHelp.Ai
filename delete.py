...
def get_main_menu(user_id):
    ...
    buttons = [
        ...
        # NEW: Delete Phone Number only
        [InlineKeyboardButton("📱 Delete Phone", callback_data="delete_phone", icon_custom_emoji_id=EMOJIS["mobile"][1], style="danger")],
...
# ======================= VIEW, DELETE (FULL), DELETE (PHONE ONLY) =======================
...
def delete_phone(call):
    ...
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(
        InlineKeyboardButton("✅ Yes, Delete Phone", callback_data="confirm_delete_phone"),
        InlineKeyboardButton("❌ Cancel", callback_data="back_main")
    )
    bot.edit_message_text(
        "⚠️ <b>Are you sure you want to delete your phone number?</b>\n"
        "Your verification will be reset. You will need to share your number again.\n"
        "All other data (balance, history, etc.) will remain intact.",
        chat_id, msg_id, parse_mode="HTML", reply_markup=markup
    )

def confirm_delete_phone(call):
    user_id = call.from_user.id
    chat_id = call.message.chat.id
    msg_id = call.message.message_id
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("UPDATE users SET phone_number = NULL, verified = 0 WHERE id = ?", (user_id,))
        conn.commit()
        bot.edit_message_text(
            f"{ce('success')} <b>Your phone number has been deleted successfully.</b>\n"
            "You can now use /start to verify again.",
            chat_id, msg_id, parse_mode="HTML"
        )
    except Exception as e:
        conn.rollback()
...
def callback_handler(call):
    ...
    if is_banned(user_id) and call.data not in ["support", "back_main", "profile", "view_my_data", "delete_my_data", "delete_phone"]:
    ...
    if data == "view_my_data":
        ...
        delete_phone(call)
    elif data == "confirm_delete_phone":
        confirm_delete_phone(call)
    # ---- Existing cases ----
    elif data == "shop_now":
        show_products(call)
    elif data == "profile":
